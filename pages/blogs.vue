...
try {
  const { data, error: fetchError } = await useMicroCMSGetList<Blog>({
    endpoint: "blog",
  });

  if (fetchError.value) {
    console.error("Error fetching blog data:", fetchError.value);
    error.value = fetchError.value;
  } else {
    blogs.value = data.value?.contents || null;
  }
} catch (err) {
...
<template>
  <div id="main">
    ...
    <article>
      <ul class="blogs fup" v-if="!isLoading && blogs && blogs.length">
        <li v-for="blog in blogs" :key="blog.id" class="blog-data">
      ...
      <p v-else-if="!isLoading && error">コンテンツの読み込みに失敗しました。再読み込みしてください。（command/control + R）</p>
      <p v-else>読み込み中...</p>
    </article>
  </div>
</template>
